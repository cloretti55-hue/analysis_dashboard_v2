...
FIXED_INCOME_OUTPUT_PATH = ROOT / "data" / "fixed-income-performance.json"
...
def write_fixed_income_snapshot(output_items: list[dict], expected_count: int) -> None:
    fixed_income_items = [
        item
    ...
    if len(fixed_income_items) != expected_count:
        print(
            "Keeping existing fixed-income snapshot "
            f"(complete={len(fixed_income_items)}/{expected_count})."
        )
        return
    ...
    FIXED_INCOME_OUTPUT_PATH.write_text(
        json.dumps(payload, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    print(f"Wrote {FIXED_INCOME_OUTPUT_PATH}")
...
def main() -> None:
    ...
    write_fixed_income_snapshot(output_items, expected_fixed_income_count)
